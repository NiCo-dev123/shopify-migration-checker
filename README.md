...
On the `dev-password` branch only, a password-protected Shopify storefront can
be checked by setting `storefront_password` in the ignored local
`inputs/urls.csv` file, or by providing the password only for the command:
